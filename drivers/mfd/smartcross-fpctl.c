...
#define FPCTL_REG_BUTTON_VALUE 40
...
#define FPCTL_INT_BUTTON_SHIFT 3
#define FPCTL_INT_BUTTON (1 << FPCTL_INT_BUTTON_SHIFT)
					   ...
					   enum led_brightness brightness)
{
	...
	struct smartcross_fpctl_priv *priv_data =
		container_of(mc_cdev, struct smartcross_fpctl_priv, mcled_cdev);
	uint8_t dat[3], i;
...
static irqreturn_t smartcross_fpctl_irq(int irq_no, void *handle)
{
	...
	uint8_t ir_data[4];
	struct smartcross_fpctl_priv *priv_data =
		(struct smartcross_fpctl_priv *)handle;

	regmap_read(priv_data->regmap, FPCTL_REG_INT_STATUS, &irq);
	if (irq & FPCTL_INT_IR_RECEIVED) {
		dev_dbg(priv_data->dev, "IR Recv\n");
		regmap_bulk_read(priv_data->regmap, FPCTL_REG_IR_RX0, ir_data,
				 sizeof(ir_data));
		if (is_rev(ir_data[0], ir_data[1]) &&
		    is_rev(ir_data[2], ir_data[3])) {
			// NEC Protocol
			rc_keydown(priv_data->rc_dev, RC_PROTO_NEC,
				   RC_SCANCODE_NEC(ir_data[0], ir_data[2]), 0);
		} else if (is_rev(ir_data[2], ir_data[3])) {
			// NECX Protocol
			rc_keydown(priv_data->rc_dev, RC_PROTO_NECX,
				   RC_SCANCODE_NECX(ir_data[0] << 8 |
							    ir_data[1],
						    ir_data[2]),
				   0);
		}
	...
	if (irq & FPCTL_INT_BUTTON) {
		int val;
		regmap_read(priv_data->regmap, FPCTL_REG_BUTTON_VALUE, &val);
		input_report_key(priv_data->rotary, BTN_0, val);
		input_sync(priv_data->rotary);
	}
...
static bool smartcross_fpctl_readable_register(struct device *dev,
					       unsigned int reg)
{
	switch (reg) {
	case 0 ... 5:
...
static bool smartcross_fpctl_writeable_register(struct device *dev,
						unsigned int reg)
{
	switch (reg) {
	case 0 ... 5:
...
static bool smartcross_fpctl_volatile_register(struct device *dev,
					       unsigned int reg)
{
	switch (reg) {
	case 33 ... 40:
				      ...
				      const struct i2c_device_id *id)
{
	...
	struct led_init_data ws2812_init_data = {
		.default_label = "ws2812", .devicename = dev_name(&client->dev)
	};

	priv_data = devm_kzalloc(
		&client->dev, sizeof(struct smartcross_fpctl_priv), GFP_KERNEL);
	if (!priv_data)
		return -ENOMEM;
	...
	priv_data->regmap =
		devm_regmap_init_i2c(client, &smartcross_fpctl_regmap);
	...
	priv_data->rc_dev =
		devm_rc_allocate_device(&client->dev, RC_DRIVER_SCANCODE);
	...
	priv_data->rc_dev->allowed_protocols =
		RC_PROTO_BIT_NECX | RC_PROTO_BIT_NEC;
...
#define NAME_LEN 20
	...
	props.brightness = 8;
	props.max_brightness = 17;

	priv_data->backlight = devm_backlight_device_register(
		&client->dev, "smartcross-backlight", &client->dev, priv_data,
	...
	input_set_capability(priv_data->rotary, EV_KEY, BTN_0);

	ret = input_register_device(priv_data->rotary);
	if (ret) {
		dev_err(&client->dev,
			"Failed to register rotary encoder device: %d\n", ret);
		return ret;
	}
	...
	priv_data->mcled_cdev.led_cdev.brightness_set_blocking =
		smartcross_fpled_brightness_set;
	ret = devm_led_classdev_multicolor_register_ext(
		&client->dev, &priv_data->mcled_cdev, &ws2812_init_data);
	if (ret) {
	...
	regmap_bulk_read(priv_data->regmap, FPCTL_REG_ROT_LOW,
			 &priv_data->rotary_last_pos, 2);
	regmap_write(priv_data->regmap, FPCTL_REG_INT_ACK, 0xFF);
	regmap_write(priv_data->regmap, FPCTL_REG_INT_ENABLE,
		     FPCTL_INT_IR_RECEIVED | FPCTL_INT_IR_REPEAT |
			     FPCTL_INT_ROTARY | FPCTL_INT_BUTTON);

	ret = devm_request_threaded_irq(&client->dev,
					gpiod_to_irq(priv_data->irq), NULL,
					smartcross_fpctl_irq,
	...
	dev_info(&client->dev,
		 "SmartCross front panel controller initialized\n");
