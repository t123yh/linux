...
static int ma120x0p_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
    ...
    for (i = 0; i < NUMBER_OF_CHIPS; i++) {
        if (priv_data->is_present[i]) {
            ...
            ENSURE_SUCCESS(i2c_smbus_write_byte_data(priv_data->clients[i], 0x35, iface | 0b1000));
            ENSURE_SUCCESS(i2c_smbus_write_byte_data(priv_data->clients[i], 0x36, pol));
        }
    }
...
static int ma120x0p_complex_probe(struct platform_device *pdev)
{
    ...
    for (i = 0; i < NUMBER_OF_CHIPS; i++) {
        priv_data->clients[i] = i2c_new_dummy_device(i2c_adapter, 0x20 + i);
    ...
    for (i = 0; i < NUMBER_OF_CHIPS; i++) {
        val = i2c_smbus_read_byte_data(priv_data->clients[i], 0x01);
        dev_dbg(&pdev->dev, "MA12070P probe got %d!!!\n", val);
        if (val == -ENXIO || val == -ENODEV) {
            continue;
        } else if (val < 0) {
            return val;
        } else if (val == 0x3C) {
