                                        ...
                                        bool is_top_level_node)
{
        ...
        dai_link->dai_fmt = SND_SOC_DAIFMT_CBS_CFS | SND_SOC_DAIFMT_I2S | SND_SOC_DAIFMT_GATED;

        if (!of_property_read_u32(node, "mclk-fs", &val))
                dai_props->mclk_fs = val;

        dai_link->cpus = devm_kzalloc(dev,
        ...
        if (!dai_link->cpus->dai_name || !dai_link->codecs->dai_name) {
                ret = -EINVAL;
                goto dai_link_of_err;
        }
        ...
        if (plat) {
                ...
                ret = of_parse_phandle_with_args(plat, "sound-dai",
                                                 "#sound-dai-cells", 0, &args);
                dai_link->platforms->of_node = args.np;
        } else {
        ...
        name = devm_kzalloc(dev,
                            strlen(dai_link->cpus->dai_name)   +
                            strlen(dai_link->codecs->dai_name) + 2,
                            GFP_KERNEL);
        if (!name) {
                ret = -ENOMEM;
                goto dai_link_of_err;
        ...
        sprintf(name, "%s-%s", dai_link->cpus->dai_name,
        ...
        dev_dbg(dev, "link_of parsing Done1!!!\n");

dai_link_of_err:
        of_node_put(cpu);
        of_node_put(codec);
        dev_dbg(dev, "link_of parsing Done2!!!\n");
                                     ...
                                     struct complex_card_data *priv)
{
        ...
        priv->mclk_fs = 128;
        ret = of_property_read_u32(node, "complex-audio-card,mclk-fs", &val);
        if (ret == 0) {
                if ((val >= 64) && (val % 2 == 0))
                        priv->mclk_fs = val;
        }
