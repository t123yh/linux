					   ...
					   unsigned int lrck_freq)
{
	...
	/* It's 128 times higher than a high sample rate */
	...
	if (ret < 0) {
		...
		dev_err(i2s_tdm->dev, "parent: %s set freq: %d ok\n",
			__clk_get_name(mclk_parent),
			mclk_parent_freq);
				     ...
				     struct clk **mclk)
{
	...
	if (i2s_tdm->clk_trcm) {
		...
		*mclk = i2s_tdm->mclk_tx;
		...
		dev_err(i2s_tdm->dev, "Set mclk_%s: %s freq: %d ok %d\n",
			substream->stream ? "rx" : "tx",
			__clk_get_name(*mclk), mclk_freq, ret);
	}
				      ...
				      struct snd_soc_dai *dai)
{
	...
	if (i2s_tdm->is_master_mode) {
		...
		printk("MCLK rate is %d, BCLK rate is %d\n", mclk_rate, bclk_rate);
